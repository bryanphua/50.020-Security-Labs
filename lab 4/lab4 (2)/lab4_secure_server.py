...
def search():
    ...
    return render_template('main.html',
                            name=session['username'][0],
                            error="Search not implemented yet. Could not find " + term,
                            news=getNews())
...
if __name__ == '__main__':
    ...
    c.execute("insert into users (email, name,password) \
              values ('alice@alice.com','alice','"
              + alicehash+"')")
    conn.commit()
